...
class TestApplication:
    @pytest.mark.parametrize("route", ["/", "/.well-known", "/whatever"])
    ...
    def test_invalid_resource(self, monkeypatch: pytest.MonkeyPatch):
        ...
        assert response_dict["error"]["message"] == "The request is not a valid Webfinger request"
        assert (
            response_dict["error"]["details"]
            == 'The `resource` query parameter must be follow the "acct" URI scheme (cf. RFC 7565)'
        )
