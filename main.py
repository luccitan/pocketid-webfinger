...
async def lifespan(app: FastAPI):
    ...
    if not pocketid_domain:
        logger.error("Missing HTTP endpoint env. variable at application initialization")
        raise RuntimeError(
            f"A valid HTTP endpoint must be defined in {POCKETID_ENDPOINT_ENVVAR} env. variable to target the OIDC endpoint"
        )

    if not RE_POCKETID_HTTPS_ENDPOINT.match(pocketid_domain):
        logger.error("Invalid HTTP endpoint env. variable found at application initialization")
        raise RuntimeError(
            f"The OIDC endpoint provided in {POCKETID_ENDPOINT_ENVVAR} env. variable must be a valid HTTPS URI"
        )
